...
export default {
  ...
  applyCellStyle(htmlCell, excelCell) {
    ...
    // 查找带有样式的子元素（优先级最高）
    let styleElement = htmlCell;
    const styledChildren = htmlCell.querySelectorAll('[style]');
    if (styledChildren.length > 0) {
      // 使用第一个带有style属性的子元素
      styleElement = styledChildren[0];
    }
    ...
    // 还要检查是否有带颜色的span或其他元素
    const coloredElements = htmlCell.querySelectorAll('*[style*="color"]');
    let textColor = '';
    if (coloredElements.length > 0) {
      // 获取最内层的带颜色的元素
      const coloredElement = coloredElements[coloredElements.length - 1];
      textColor = coloredElement.style.color;
    }
    
    // 字体颜色 - 优先使用直接在标签上的style属性
    if (textColor) {
      const argb = this.colorToARGB(textColor);
      excelCell.font.color = { argb: argb };
      console.log('文字颜色(从内联元素):', textColor, '-> ARGB:', argb);
    } else if (inlineStyle.color) {
      const argb = this.colorToARGB(inlineStyle.color);
      excelCell.font.color = { argb: argb };
      console.log('文字颜色(内联样式):', inlineStyle.color, '-> ARGB:', argb);
    } else if (computedStyle.color && computedStyle.color !== 'rgb(0, 0, 0)') {
      const argb = this.colorToARGB(computedStyle.color);
      excelCell.font.color = { argb: argb };
      console.log('文字颜色(计算样式):', computedStyle.color, '-> ARGB:', argb);
    }
    ...
    if (inlineStyle.backgroundColor && inlineStyle.backgroundColor !== 'transparent') {
      ...
      console.log('背景色(内联样式):', inlineStyle.backgroundColor, '-> ARGB:', argb);
      ...
      console.log('背景色(计算样式):', computedStyle.backgroundColor, '-> ARGB:', argb);
    }
